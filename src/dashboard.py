...
with st.sidebar:
    ...
    _playlist_html = (
        ...
        '</audio>'
        '<script>'
        'var playlist = [' + _sources + '];'
        'var idx = 0;'
        'var a = document.getElementById("f1audio");'
        'if (a) { a.volume = 0.5; }'
        'function playNext() { idx = (idx + 1) % playlist.length; a.src = playlist[idx]; a.play().catch(function(){}); }'
        'if (a) { a.addEventListener("ended", playNext); a.play().catch(function(){}); }'
    ...
    st.markdown(_playlist_html, unsafe_allow_html=True)
